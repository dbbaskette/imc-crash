...
export const AgentPortal: React.FC = () => {
  ...
  const fetchMessages = useCallback(async () => {
    try {
      const data = await messageAPI.getAdjusterMessages();
      setMessages(data);
      setSelectedIds(prev => {
      ...
      console.error('Failed to fetch adjuster messages:', error);
    } finally {
      setLoading(false);
    }
  }, []);
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-700/50 bg-slate-800/30">
          <label className="flex items-center gap-2 cursor-pointer text-sm text-slate-300 hover:text-white transition-colors">
            <input
              ...
              className="w-4 h-4 rounded border-slate-600 bg-slate-700 text-blue-500 focus:ring-blue-500 focus:ring-offset-0 cursor-pointer disabled:opacity-50"
            />
            <span>All</span>
          </label>
          <button
        ...
        <div className="flex-1 overflow-y-auto">
          {messages.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center p-6">
              ...
              <p className="text-slate-400 font-medium">No FNOL emails yet</p>
            ...
            messages.map((msg) => (
              ...
              >
                <input
                  ...
                  onChange={() => {}}
                  className="w-4 h-4 mt-0.5 rounded border-slate-600 bg-slate-700 text-blue-500 focus:ring-blue-500 focus:ring-offset-0 cursor-pointer flex-shrink-0"
                />
